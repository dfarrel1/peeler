...
pub fn is_udp_packet(packet: Packet) -> bool {
    ...
    let protocol: u8 = match ether_type {
        ...
        0x86F7 => { // IPv4 over IPsec
            let ip_versioned_header = Ipv4HeaderSlice::from_slice(ip_header);
            ip_versioned_header.unwrap().protocol()
        }
        0x8100 => { // IPv4 over VLAN
            let ip_versioned_header = Ipv4HeaderSlice::from_slice(ip_header);
            ip_versioned_header.unwrap().protocol()
        }
        _ => {
...
pub fn extract_udp_fields(packet: Packet) -> Result<Value, Box<dyn Error>> {
    ...
    let src_port = u16::from_be_bytes([packet.data[udp_header_offset], packet.data[udp_header_offset + 1]]);
    let dst_port = u16::from_be_bytes([packet.data[udp_header_offset + 2], packet.data[udp_header_offset + 3]]);
    let udp_len = u16::from_be_bytes([packet.data[udp_header_offset + 4], packet.data[udp_header_offset + 5]]);
    let checksum = u16::from_be_bytes([packet.data[udp_header_offset + 6], packet.data[udp_header_offset + 7]]);

    let udp_data_offset = udp_header_offset + 8; // UDP header is 8 bytes long
    let udp_data_len = (udp_len as usize) - 8; // Subtract header length to get data length
